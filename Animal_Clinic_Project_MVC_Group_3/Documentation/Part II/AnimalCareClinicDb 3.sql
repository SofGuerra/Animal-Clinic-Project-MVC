...
Role NVARCHAR(20) CHECK (Role IN ('Admin', 'Veterinarian', 'Receptionist')) NOT NULL
);

CREATE TABLE Administrator (
UserID INT PRIMARY KEY FOREIGN KEY REFERENCES [ClinicUser](UserID)
);
...
INSERT INTO ClinicUser (UserID, FirstName, LastName, Email, Password, Role) VALUES
(1, 'Admin',    'Root',     'admin@clinic.com',  '1234', 'Admin'),
(2, 'Ana',      'García',   'ana@clinic.com',    '1234', 'Veterinarian'),
(3, 'Luis',     'Ruiz',     'luis@clinic.com',   '1234', 'Veterinarian'),
(4, 'Marta',    'López',    'marta@clinic.com',  '1234', 'Receptionist');


INSERT INTO Administrator (UserID) VALUES (1);


INSERT INTO Veterinarian (UserID, Speciality, Availability) VALUES
(2, 'Small animals', 'Mon-Fri 09-17'),
(3, 'Exotics',       'Tue-Thu 10-16');

INSERT INTO Receptionist (UserID, Phone) VALUES
(4, '555-1234');

INSERT INTO Owner (OwnerID, FirstName, LastName, Email, Phone, Pet) VALUES
(10, 'Carlos',  'Pérez', 'cp@mail.com', 5551111, 'Dog'),
(11, 'Lucía',   'Díaz',  'ld@mail.com', 5552222, 'Cat'),
(12, 'Sonia',   'Sánchez','ss@mail.com',5553333, 'Rabbit');

INSERT INTO Pet (PetID, Name, Species, Age, OwnerID, Appointments) VALUES
(100, 'Toby',  'Dog',   3, 10, NULL),
(101, 'Misha', 'Cat',   2, 11, NULL),
(102, 'Luna',  'Rabbit',1, 12, NULL);
...
AS SELECT v.UserID AS VeterinarianID,
u.FirstName + '' + u.LastName AS VetsName,
a.Time as SlotTime, a.AppointmentID, 
p.Name as PetName, o.FirstName + '' + o.LastName as OwnersName,
a.Status
FROM Veterinarian v
JOIN ClinicUser u ON u.UserID = v.UserID
CROSS JOIN TimeSlot t
LEFT JOIN Appointment a ON a.VeterinarianID = u.UserID
AND a.Date = CAST(GETDATE() AS DATE) AND
a.Time = t.Slot
...
--3. Clinic History
CREATE VIEW v_pet_history
AS SELECT p.PetID, p.Name AS PetName,
p.Species, p.Age, o.FirstName + '' + o.LastName as OwnersName,
ch.Date, ch.Description, ch.Treatment, vet.FirstName + '' + vet.LastName AS VetName
FROM Pet p
...
--4. Slots Available
CREATE VIEW vets_availability
AS SELECT v.UserID, u.FirstName + ' ' + u.LastName AS VetName,
t.Slot
FROM   Veterinarian v
JOIN   ClinicUser   u ON u.UserID = v.UserID
CROSS JOIN TimeSlot t
...
CREATE TRIGGER delete_pets_from_owner
ON Owner
AFTER DELETE
AS
BEGIN
...
--2 When deleting a pet delete history and appointments
CREATE TRIGGER delete_pet_data
ON Pet
AFTER DELETE
AS
BEGIN
...
--4 When owner is deleted, pet appointments are cancelled

CREATE TRIGGER cancel_future_appointments
ON Owner
AFTER DELETE
AS
BEGIN
...
--2
SELECT * FROM Appointment WHERE PetID = 101;
